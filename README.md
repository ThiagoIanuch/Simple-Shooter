# Simple Shooter

Simple Shooter is a third person shooter game developed in Unreal Engine 5 following part 5/5 of the course "<a href="https://www.udemy.com/course/unrealcourse/">Unreal Engine 5 C++ Developer: Learn C++ & Make Video Games</a>" by Ben Tristem and GameDev.tv Team.

<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/aa929031-b004-4481-8fa8-5bb35fc114aa" />


https://github.com/user-attachments/assets/e116cb04-a7d7-4f06-a939-9c1962607f53

## Build with
![Unreal Engine](https://img.shields.io/badge/Unreal%20Engine%205-%23313131.svg?style=for-the-badge&logo=unrealengine&logoColor=white) ![C++](https://img.shields.io/badge/C++-%235C2D91.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white) ![Blueprints](https://img.shields.io/badge/Blueprints-blue?style=for-the-badge&logo=unrealengine&logoColor=white)

## How to download and play
1. Download the game on my <a href="https://thiagoianuch.itch.io/simple-shooter">itch.io</a> **(Windows only)**

<div align="center">
    <a href="https://thiagoianuch.itch.io/simple-shooter">
        <img src="http://jessemillar.github.io/available-on-itchio-badge/badge-bw.png" alt="Available on itch.io">
    </a>
</div>

 
2. Unzip the folder and run SimpleShooter.exe

<br>

**Controls:**
* WASD - Move
* Space - Jump
* Left Click - Shoot
